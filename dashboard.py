...
# Page config
st.set_page_config(page_title="Batterie-Analyse für Alloheim Standorte", layout="wide", initial_sidebar_state="expanded")
...
def main():
    ...
    if page == "📊 Übersicht":
        ...
        if selected:
            ...
            battery_cost = 52000
            amortization_no_pv = battery_cost / location_data['savings_no_pv'] if location_data['savings_no_pv'] > 0 else float('inf')
            ...
            **Batterie-Spezifikationen:**
            ...
            with st.expander("📊 Detaillierte Statistiken", expanded=False):
                ...
                with col1:
                    ...
                    st.write(f"**Min SoC:** {df_detail['battery_soc'].min():.1f} kWh ({df_detail['battery_soc'].min()/BATTERY_CAPACITY*100:.1f}%)")
                    st.write(f"**Zyklen (geschätzt):** {(df_detail['battery_discharge'].sum() * 0.25 / BATTERY_CAPACITY):.1f}")
